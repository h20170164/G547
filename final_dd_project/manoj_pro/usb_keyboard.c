...
void keypress_scan(struct input_polled_dev *usbkeypolldev)
{
struct input_dev *myinputdev = usbkeypolldev->input;
spin_lock(&lock);
	
usb_control_msg(usbkeydev,usb_rcvctrlpipe(usbkeydev,0),0,USB_TYPE_VENDOR|USB_DIR_IN,0,0,&buffer,1,0);

printk(KERN_INFO "buffer %u \n",buffer);

input_report_key(myinputdev, KEY_S, buffer);
...
{	
	...
	usbkeypolldev->poll = keypress_scan;
	usbkeypolldev->poll_interval = 500;  //msec
...
static void keypress_disconnect(struct usb_interface *interface)
{
...
static struct usb_driver keypress_usb =
{
	...
	.disconnect = keypress_disconnect,
